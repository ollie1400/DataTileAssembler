 ...
 */
PNGReader.prototype.decodeChunk = function(options){
	...
	switch (type){
		...
		case 'IDAT': if (options.data != false) this.decodeIDAT(chunk); break;
...
PNGReader.prototype.decodeTEXT = function(chunk){
	// extract the key
	var keyEndPos = chunk.indexOf(0);
	var key = bufferToString(chunk.slice(0,keyEndPos));
	
	// value
	var value = bufferToString(chunk.slice(keyEndPos+1, chunk.length-1));
	
	// multiple IDAT chunks will concatenated
	this.textChunks.push({key: key, value: value});
};
...
PNGReader.prototype.parse = function(options, callback){
	...
	try {
		...
		while (this.i < this.bytes.length){
			var type = this.decodeChunk(options);
		...
		var png = this.png;
		png.setTextChunks(this.textChunks);

		// decode pixel data if asked
